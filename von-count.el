...
in which buffer BODY is called."
  (if (equal parent-or-bar 'bar)
      `(with-current-buffer (or (and von-count-is-bar (current-buffer)) von-count-bar-buffer)
     ,@body)
    `(with-current-buffer (or (and von-count-is-parent (current-buffer)) von-count-parent-buffer)
     ,@body)))
...
default the word count is of the whole buffer"
  (von-count-wrapper
   parent
   (message "Inital word-count is %s" (setq-local von-count-initial-word-count (von-count--get-word-count pos)))))
